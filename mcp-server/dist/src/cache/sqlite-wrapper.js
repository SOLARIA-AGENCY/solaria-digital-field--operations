...
export function getProjectCachePath(projectId, cacheDir) {
...
export class SQLiteCache {
    ...
    async cacheTasks(tasks) {
        ...
        for (const task of tasks) {
            await this.db.run(`INSERT OR REPLACE INTO tasks
    ...
    async updateCachedTask(taskId, updates) {
        ...
        params.push(taskId);
        await this.db.run(`UPDATE tasks SET ${setClauses.join(', ')} WHERE id = ?`, params);
    }
    // ============================================================================
    // Sync Metadata
    // ============================================================================
    async setMeta(key, value) {
        if (!this.db)
            throw new Error('Database not initialized');
        await this.db.run(`INSERT OR REPLACE INTO sync_meta (key, value, updated_at) VALUES (?, ?, ?)`, [key, value, new Date().toISOString()]);
    }
    async getMeta(key) {
        if (!this.db)
            throw new Error('Database not initialized');
        const row = await this.db.get('SELECT value FROM sync_meta WHERE key = ?', [key]);
        return row?.value;
    }
    ...
    async clearModifiedFlag(taskId) {
        if (!this.db)
            throw new Error('Database not initialized');
    ...
    async getStats() {
        if (!this.db)
            throw new Error('Database not initialized');
        const totalRow = await this.db.get('SELECT COUNT(*) as count FROM tasks');
        const modifiedRow = await this.db.get('SELECT COUNT(*) as count FROM tasks WHERE local_modified = 1');
