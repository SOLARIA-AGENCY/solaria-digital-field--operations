...
// Helper Functions
// ============================================================================
function generateReadinessReasons(task) {
    const reasons = [];
...
function formatReadyTasks(tasks) {
    if (tasks.length === 0) {
        return `
📋 Ready Tasks: None
...
Use /dfo status to see all tasks.
    ...
    tasks.forEach((task, index) => {
        ...
        lines.push(`${index + 1}. ${priorityIcon} ${task.task_code}: ${task.title}`, `   Readiness: ${task.readiness_score}/100 | Priority: ${task.priority}`);
        if (task.agent_name) {
    ...
    async execute(params) {
      ...
      `;
          ...
          `;
            ...
            const tasksWithReasons = tasks.map((task) => ({
                ...task,
            ...
            const formatted = params.format === 'human' ? formatReadyTasks(tasksWithReasons) : undefined;
...
//# sourceMappingURL=ready-tasks.js.map