 ...
 */
async function getTasksByStatus(projectId, dateFrom, dateTo) {
    const query = `
    SELECT
      COUNT(*) as total,
 ...
 */
async function getTasksByPriority(projectId) {
    const query = `
    SELECT
 ...
 * Get velocity data
 */
async function getVelocity(projectId, sprintId) {
  ...
  `;
    ...
    const history = historyResults.map((r) => ({
 ...
 */
async function getAgentWorkload(projectId) {
    const query = `
    SELECT
      a.id as agent_id,
  ...
  `;
    ...
    const active = results.filter((r) => r.agent_status === 'active').length;
 ...
 */
function calculateHealthScore(tasksByStatus, total, velocity) {
    if (total === 0)
        return 100;
    // Completion rate (30%)
    ...
    return Math.round(completionScore + blockedScore + velocityScore + utilizationScore + progressScore);
}
// ============================================================================
    ...
    async execute(params) {
        ...
        try {
            ...
            const completionRate = taskData.total > 0
                ...
                : 0;
            // Calculate health score
            const healthScore = calculateHealthScore(taskData.by_status, taskData.total, velocityData);
            const statsData = {
