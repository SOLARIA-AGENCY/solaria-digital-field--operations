...
// ============================================================================
const VERSION = '2.0.0';
const DOCUMENT_TYPES = ['plan', 'spec', 'report', 'manual', 'adr', 'roadmap', 'audit', 'other'];
// ============================================================================
// Validation Schemas
// ============================================================================
 ...
 */
async function getDocumentById(documentId, projectId, includeContent = true) {
 ...
 */
async function isDuplicateName(name, projectId, excludeId) {
    const query = `
    ...
    async execute(params) {
        ...
        try {
            ...
            if (await isDuplicateName(params.name, params.project_id)) {
                return builder.error('DUPLICATE_DOCUMENT', `A document with name "${params.name}" already exists`, { name: params.name });
            }
            const query = `
 ...
 */
export const getInlineDocument = {
    ...
    async execute(params) {
        const builder = new ResponseBuilder({ version: VERSION });
        try {
            const document = await getDocumentById(params.document_id, params.project_id, params.include_content);
            if (!document) {
                return builder.error('DOCUMENT_NOT_FOUND', `Document with ID ${params.document_id} not found`, { document_id: params.document_id });
            }
 ...
 */
export const listInlineDocuments = {
 ...
 */
export const updateInlineDocument = {
    ...
    async execute(params) {
        ...
        try {
            ...
            // Archive old version
            await db.execute('UPDATE inline_documents SET is_active = 0, archived_at = NOW() WHERE id = ?', [params.document_id]);
            // Insert new version
            const insertQuery = `
        INSERT INTO inline_documents (
 ...
 */
export const deleteInlineDocument = {
    ...
    async execute(params) {
        ...
        try {
            ...
            const existing = await getDocumentById(params.document_id, params.project_id);
            if (!existing) {
                return builder.error('DOCUMENT_NOT_FOUND', `Document with ID ${params.document_id} not found or already deleted`, { document_id: params.document_id });
            }
            // Soft delete
            await db.execute('UPDATE inline_documents SET is_active = 0, archived_at = NOW() WHERE id = ?', [params.document_id]);
            return builder.success({
...
//# sourceMappingURL=inline-documents.js.map