...
describe('get_ready_tasks Endpoint', () => {
    ...
    describe('Readiness Score Calculation', () => {
        it('should prioritize critical priority tasks', async () => {
            ...
            expect(result.success).toBe(true);
            if (result.success) {
                expect(result.data.tasks[0].readiness_score).toBeGreaterThan(result.data.tasks[1].readiness_score);
            }
        });
        ...
        it('should boost assigned tasks', async () => {
            ...
            db.query.mockResolvedValue([taskWithAgent, taskUnassigned]);
            const result = await getReadyTasks.execute({ format: 'json' });
            expect(result.success).toBe(true);
            if (result.success) {
                const withAgent = result.data.tasks.find((t) => t.id === 1);
                const withoutAgent = result.data.tasks.find((t) => t.id === 2);
    ...
    describe('Readiness Reasons', () => {
        it('should generate appropriate readiness reasons', async () => {
            ...
            if (result.success) {
                ...
                expect(task.readiness_reasons.some((r) => r.includes('Deadline in'))).toBe(true);
            }
        });
        it('should mark overdue tasks', async () => {
            ...
            if (result.success) {
                const task = result.data.tasks[0];
                expect(task.readiness_reasons.some((r) => r.includes('⚠ OVERDUE'))).toBe(true);
            }
        });
    });
...
//# sourceMappingURL=ready-tasks.test.js.map