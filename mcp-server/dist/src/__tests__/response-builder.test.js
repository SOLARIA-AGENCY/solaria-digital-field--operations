...
import { ResponseBuilder, successResponse, errorResponse, wrapEndpoint, CommonErrors, validateResponse, } from '../utils/response-builder';
...
describe('Helper Functions', () => {
    describe('successResponse', () => {
        ...
        it('should support format parameter', () => {
            const response = successResponse({ test: true }, { format: 'human', formatted: 'Test output' });
            expect(response.format).toBe('human');
    ...
    describe('wrapEndpoint', () => {
        ...
        it('should apply formatter for human format', async () => {
            ...
            const formatter = (data) => `Found ${data.count} items`;
            const response = await wrapEndpoint(executor, {
                format: 'human',
                formatter,
            });
            expect(response.success).toBe(true);
            expect(response.formatted).toBe('Found 5 items');
        });
        it('should include version in metadata', async () => {
            ...
            expect(response.success).toBe(true);
            expect(response.metadata.version).toBe('3.0.0');
        });
    });
});
