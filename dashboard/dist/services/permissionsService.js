...
exports.PermissionsService = exports.PERMISSION_CODES = void 0;
exports.createRequirePermission = createRequirePermission;
...
class PermissionsService {
     ...
     */
    async hasPermission(role, permissionCode) {
     ...
     */
    async hasAnyPermission(role, permissionCodes) {
        const permissions = await this.getRolePermissions(role);
     ...
     */
    async hasAllPermissions(role, permissionCodes) {
        const permissions = await this.getRolePermissions(role);
    ...
    async getAllPermissions() {
        const [rows] = await this.db.execute('SELECT * FROM permissions ORDER BY category, code');
        return rows;
    }
    /**
    ...
    async getPermissionsByCategory(category) {
        const [rows] = await this.db.execute('SELECT * FROM permissions WHERE category = ? ORDER BY code', [category]);
        return rows;
    }
    /**
     * Get all role-permission mappings for a role
     */
    async getRolePermissionMappings(role) {
        const [rows] = await this.db.execute(`
     ...
     */
    async addRolePermission(role, permissionId) {
        await this.db.execute('INSERT IGNORE INTO role_permissions (role, permission_id) VALUES (?, ?)', [role, permissionId]);
        cacheExpiry = 0;
    }
    /**
     * Remove a permission from a role
     */
    async removeRolePermission(role, permissionId) {
        await this.db.execute('DELETE FROM role_permissions WHERE role = ? AND permission_id = ?', [role, permissionId]);
        cacheExpiry = 0;
    }
    // ========================================================================
    // User Preferences
    ...
    async getUserPreferences(userId) {
        const [rows] = await this.db.execute('SELECT * FROM user_preferences WHERE user_id = ?', [userId]);
        if (rows.length === 0) {
            return null;
        }
        const row = rows[0];
    ...
    async updateUserPreferences(userId, preferences) {
        ...
        if (existing) {
            ...
            if (updates.length > 0) {
                values.push(userId);
                await this.db.execute(`UPDATE user_preferences SET ${updates.join(', ')} WHERE user_id = ?`, values);
            }
        }
        else {
            await this.db.execute(`INSERT INTO user_preferences (user_id, default_view, sidebar_collapsed, theme, notifications_enabled, email_notifications, dashboard_widgets)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                userId,
...
function createRequirePermission(db) {
    ...
    return function requirePermission(...permissions) {
