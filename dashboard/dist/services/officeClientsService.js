...
exports.OfficeClientsService = void 0;
class OfficeClientsService {
    db;
    constructor(db) {
        this.db = db;
    }
    ...
    async getClients(filters = {}) {
        ...
        // Get total count
        const [countResult] = await this.db.execute(`SELECT COUNT(*) as total FROM office_clients c WHERE ${whereClause}`, params);
    ...
    async updateClient(id, data) {
        ...
        values.push(id);
        await this.db.execute(`UPDATE office_clients SET ${updates.join(', ')} WHERE id = ?`, values);
        return true;
    }
    /**
    ...
    async deleteClient(id) {
        const [result] = await this.db.execute('UPDATE office_clients SET status = ? WHERE id = ?', ['churned', id]);
    ...
    async getClientContacts(clientId) {
        const [rows] = await this.db.execute('SELECT * FROM office_client_contacts WHERE client_id = ? ORDER BY is_primary DESC, name ASC', [clientId]);
        return rows.map(row => ({
            ...row,
    ...
    async getContact(contactId) {
        const [rows] = await this.db.execute('SELECT * FROM office_client_contacts WHERE id = ?', [contactId]);
        if (rows.length === 0)
            return null;
        return {
            ...rows[0],
    ...
    async updateContact(contactId, data) {
        ...
        if (data.is_primary) {
            ...
            if (contact) {
                await this.db.execute('UPDATE office_client_contacts SET is_primary = 0 WHERE client_id = ? AND id != ?', [contact.client_id, contactId]);
            }
        }
        const updates = [];
        const values = [];
        ...
        await this.db.execute(`UPDATE office_client_contacts SET ${updates.join(', ')} WHERE id = ?`, values);
        return true;
    }
    /**
     * Delete a contact
    ...
    async deleteContact(contactId) {
        const [result] = await this.db.execute('DELETE FROM office_client_contacts WHERE id = ?', [contactId]);
        return result.affectedRows > 0;
    }
    // ========================================================================
    // Client Projects
     ...
     */
    async assignProjectToClient(clientId, projectId) {
        const [result] = await this.db.execute('UPDATE projects SET office_client_id = ? WHERE id = ?', [clientId, projectId]);
     ...
     */
    async removeProjectFromClient(projectId) {
        // Get current client before removing
        const [projects] = await this.db.execute('SELECT office_client_id FROM projects WHERE id = ?', [projectId]);
        const clientId = projects[0]?.office_client_id;
        const [result] = await this.db.execute('UPDATE projects SET office_client_id = NULL WHERE id = ?', [projectId]);
        // Update client total projects count
     ...
     */
    async updateClientProjectCount(clientId) {
    ...
    async updatePayment(paymentId, data) {
        ...
        await this.db.execute(`UPDATE office_payments SET ${updates.join(', ')} WHERE id = ?`, values);
        // Update client lifetime value if relevant
        const [payments] = await this.db.execute('SELECT client_id FROM office_payments WHERE id = ?', [paymentId]);
     ...
     */
    async updateClientLifetimeValue(clientId) {
        await this.db.execute(`
            UPDATE office_clients
            SET lifetime_value = (
    ...
    async getClientStats() {
        const [totalResult] = await this.db.execute('SELECT COUNT(*) as total FROM office_clients');
        const [statusResult] = await this.db.execute('SELECT status, COUNT(*) as count FROM office_clients GROUP BY status');
        const [sizeResult] = await this.db.execute('SELECT company_size, COUNT(*) as count FROM office_clients GROUP BY company_size');
        const [ltvResult] = await this.db.execute('SELECT SUM(lifetime_value) as total_ltv FROM office_clients');
        return {
     ...
     */
    async getIndustries() {
        const [rows] = await this.db.execute('SELECT DISTINCT industry FROM office_clients WHERE industry IS NOT NULL ORDER BY industry');
        return rows.map(r => r.industry);
    }
}
exports.OfficeClientsService = OfficeClientsService;
exports.default = OfficeClientsService;
//# sourceMappingURL=officeClientsService.js.map